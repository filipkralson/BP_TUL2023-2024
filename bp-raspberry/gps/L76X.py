        ...
        while 1:
            ...
            if x==b'$':
                ...
                if '$GNGLL' in data:
                    break
        self.Speed=g.speed
        # Získat Latitude jako jediné float číslo
        self.Lat = int(g.latitude[0]) + float(g.latitude[1])/60
        if g.latitude[2] == 'S':
            self.Lat *= -1
        
        # Získat Longitude jako jediné float číslo
        self.Lon = int(g.longitude[0]) + float(g.longitude[1])/60
        if g.longitude[2] == 'W':
            self.Lon *= -1
        # Zjistit směr zeměpisné šířky a délky
        self.Lat_area = g.latitude[2]
        self.Lon_a = g.longitude[2]
        if g.latitude[2]!='N':
            self.Lat=self.Lat*(-1)
        if g.longitude[2]!='E':
            self.Lon=self.Lon*(-1)
